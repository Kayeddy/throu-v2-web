...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  const t = useTranslations("AboutUs.roadmap");
  ...
  return (
    <div
      className="w-full bg-transparent font-jakarta text-primary"
      ref={containerRef}
    >
      <h2 className="font-sen text-4xl font-bold text-primary">{t("title")}</h2>
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              ...
              <h3 className="hidden text-xl font-bold text-secondary md:block md:pl-20 md:text-5xl">
                {item.title}
              </h3>
            </div>
